import Validator, { castHandlers, ECastType, ERule } from '../Validator';
...
describe('Validator', () => {
  ...
  it(`rule: ${ERule.Cast}`, () => {
    castHandlers.json = (value: string) => {
      // cast only strings
    ...
    const validator = new Validator(payload, {
      ...
      'json': `${ERule.Cast}:json`, // custom parse
      'json.a': `required|in:5`, // custom parse
